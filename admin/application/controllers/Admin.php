...
class Admin extends CI_Controller {
	...
	public function index(){
		$data['title'] = 'Voting System for Amber Velasco by Pyanz gwapo';
	...
	public function voting(){
		...
		$data['listkandidat'] = $this->db->select('id_kandidat, nama_kandidat')->get('kandidat')->result();
		...
		if ($data['cekvoting'] > 0) {
			...
			$data['kandidat'] = $this->db->select('kandidat.*, ikut_kandidat.id_voting, ikut_kandidat.poin')
																	 ->where('kandidat.id_kandidat = ikut_kandidat.id_kandidat')
																	 ->join('kandidat', 'ikut_kandidat.id_voting = '.$data['voting']->id_voting, 'inner')
																	 ->get('ikut_kandidat')->result();
	...
	function get_kandidat($id){
		...
		$d = $this->db->get_where('kandidat', ['id_kandidat' => $id])->row();
		echo json_encode($d);
	}
	...
	function edit_kandidat(){
		...
		if (empty($foto['name'])) {
			...
			if (!$this->upload->do_upload('foto')) {
				...
				$f = $this->db->select('foto')->get_where('kandidat', ['id_kandidat' => $id])->row();
	...
	function hapus_kandidat($id){
		...
		$d = $this->db->select('foto')->get_where('kandidat', ['id_kandidat' => $id])->row();
	...
	function get_pemilih($id){
		...
		$d = $this->db->select('id_pemilih, nama, username')->get_where('pemilih', ['id_pemilih' => $id])->row();
		echo json_encode($d);
	}
	...
	function reset_pass_pemilih($id){
		...
		$p = $this->db->select('username')->get_where('pemilih', ['id_pemilih' => $id])->row();
		$pass = md5($p->username);
	...
	function get_admin($id){
		...
		$d = $this->db->select('id_admin, nama, username')->get_where('admin', ['id_admin' => $id])->row();
		echo json_encode($d);
	}
